...
export const registerSchemaForm = yup.object({
  ...
  password: yup.string().required('Senha é obrigatória').min(10, 'A Senha deve ter pelo menos 10 caracteres.').max(30, 'A Senha deve ter entre 10 e 30 caracteres.'),
  birthData: yup.date().required('Data de nascimento é obrigatória').max(new Date(), 'A data não pode ser no futuro.'),
  motherName: yup
    .string()
